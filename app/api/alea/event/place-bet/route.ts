...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (!followingBookies) {
    ...
    return notFollowingResponse(generateUrl(`/api/alea/${FrameNames.EVENT}/${FrameNames.PLACE_BET}`, {[RequestProps.EVENT_NAME]: eventName}, false))
  }
  ...
  if (event.startDate < now || result !== -1) {
    ...
    imageUrl = generateUrl(`api/alea/${FrameNames.EVENT}/${FrameNames.PLACE_BET}/image`, {[RequestProps.PROMPT]: event.prompt, [RequestProps.BALANCE]: user.balance, [RequestProps.TIME]: event.startDate}, true);
  }
  ...
  const frame : Frame = {
    ...
    image: imageUrl,
    postUrl: generateUrl(`api/alea/${FrameNames.EVENT}/${FrameNames.BETSLIP}`, {[RequestProps.EVENT_NAME]: eventName}, false),
  };
