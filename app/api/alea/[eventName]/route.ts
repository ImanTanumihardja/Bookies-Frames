...
export async function GET(req: NextRequest, { params: { eventName } }: { params: { eventName: string } }): Promise<Response> {
  ...
  const imageUrl:string = generateUrl(`api/alea/${eventName}/image`, {[RequestProps.EVENT_NAME]: eventName, [RequestProps.TIME]: startDate}, true)
  ...
  const frame : Frame = {
    ...
    image: imageUrl,
    postUrl: generateUrl(`api/alea/${eventName}/${FrameNames.PLACE_BET}`, {[RequestProps.EVENT_NAME]: eventName}, false),
  };
