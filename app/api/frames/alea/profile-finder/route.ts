...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (!isFollowing) {
    ...
    return notFollowingResponse(generateUrl(`/api/frames/alea/${FrameNames.PROFILE_FINDER}`, {[RequestProps.FID]: -1}, false))
  }
  ...
  let imageUrl: string = "";
  let postUrl = generateUrl(`/api/frames/alea/${FrameNames.PROFILE_FINDER}`, {[RequestProps.FID]: ''}, false)
  let offset = 0;
  let count = 5;
  ...
  imageUrl = generateUrl(`api/frames/alea/${FrameNames.PROFILE_FINDER}/image`, {[RequestProps.IS_FOLLOWING]: isFollowing, 
  ...
  const frame: Frame = {
    ...
    [
      ...
      },
      {
        label: "Bets",
        action: 'post',
        target: generateUrl(`/api/frames/alea/${FrameNames.PROFILE_FINDER}/${FrameNames.BETS}`, {[RequestProps.FID]: profile?.fid, [RequestProps.INDEX]: 0, [RequestProps.ARRAY]: eventNames}, false)
      },
      {
        label: "Leaderboard",
        action: 'post',
        target: generateUrl(`/api/frames/alea/${FrameNames.LEADERBOARD}`, {[RequestProps.OFFSET]: offset, [RequestProps.COUNT]: count}, false)
      }
    ]),
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const frame : Frame = {
    ...
    image: imageUrl,
    postUrl: generateUrl(`api/frames/alea/${FrameNames.PROFILE_FINDER}`, {[RequestProps.FID]: ''}, false),
  };
