...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (event.startDate < now || result !== -1) {
    pick = -1;
    imageUrl = generateUrl(`api/frames/alea/${FrameNames.BET_CONFIRMATION}/image`, {[RequestProps.STAKE]: stake, 
  ...
  else
  {
    ...
    imageUrl = generateUrl(`api/frames/alea/${FrameNames.BETSLIP}/image`, {[RequestProps.PICK]: pick, 
  ...
  return new NextResponse(
    getFrameHtml({
      version: "vNext",
      image: `${imageUrl}`,
      postUrl: generateUrl(`api/frames/alea/${FrameNames.BET_CONFIRMATION}`, {[RequestProps.EVENT_NAME]: eventName, [RequestProps.STAKE]: stake, [RequestProps.PICK]: button-1}, false),
      buttons: pick !== -1 ? [
                {
                  label: "Confirm", 
                  action: 'post'
