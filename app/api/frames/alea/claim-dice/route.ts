...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (!isFollowing) {
    ...
    return notFollowingResponse(generateUrl(`/api/frames/alea/${FrameNames.CLAIM_DICE}`, {[RequestProps.CAPTCHA_INDEX]: -1}, false))
  }
  ...
  const imageUrl = generateUrl(`api/frames/alea/${FrameNames.CLAIM_DICE}/image`, {[RequestProps.HAS_CLAIMED]: hasClaimed, [RequestProps.BALANCE]: isNewUser ? 100 : 10, [RequestProps.VALID_CAPTCHA]: validCaptcha}, true);

  const frame: Frame = {
    version: "vNext",
    image: imageUrl,
    buttons: !validCaptcha ? [] : [{ label: "Check out /bookies!", action: 'link', target: 'https://warpcast.com/~/channel/bookies'}],
    postUrl: generateUrl(`api/frames/alea/${FrameNames.CLAIM_DICE}`, {}, false),
  };
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const frame : Frame = {
    ...
    image: imageUrl,
    postUrl: generateUrl(`api/frames/alea/${FrameNames.CAPTCHA}`, {}, false),
  };
