...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (!followingBookies) {
    ...
    return notFollowingResponse(generateUrl(`/api/frames/exchange/${FrameNames.PLACE_BET}`, {[RequestProps.EVENT_NAME]: eventName}, false))
  }
  ...
  if (/*event.startDate < now || result !== -1*/ false) {
    // const pick = -1;
    // inputText = undefined
    ...
    //     }
    //   ]
    // imageUrl = generateUrl(`api/frames/exchange/${FrameNames.BET_CONFIRMATION}/image`, {[RequestProps.STAKE]: 0, 
  ...
  else 
  {
    buttons = event.options.map((option, index) => {
      ...
      return {
        ...
        action: 'tx',
        target: generateUrl(`api/frames/exchange/${FrameNames.TRANSACTIONS}/${FrameNames.APPROVE}`, {}, false),
      } as FrameButton
    })
    imageUrl = generateUrl(`api/frames/exchange/${FrameNames.PLACE_BET}/image`, {[RequestProps.PROMPT]: event.prompt, [RequestProps.BALANCE]: user.balance, [RequestProps.TIME]: event.startDate}, true);
  }
  ...
  const frame : Frame = {
    ...
    image: imageUrl,
    postUrl: generateUrl(`api/frames/exchange/${FrameNames.BETSLIP}`, {[RequestProps.EVENT_NAME]: eventName}, false),
  };
