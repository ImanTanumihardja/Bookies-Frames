...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (/*event.startDate < now || result !== -1*/ false) {
    ...
    //     }
    //   ]
    // imageUrl = generateUrl(`api/frames/bookies/${FrameNames.BET_CONFIRMATION}/image`, {[RequestProps.STAKE]: 0, 
  ...
  else 
  {
    buttons = event.options.map((option, index) => {
      ...
      return {
        ...
        action: 'tx',
        target: generateUrl(`api/frames/bookies/${FrameNames.TRANSACTIONS}/${FrameNames.APPROVE}`, {}, false),
      } as FrameButton
    })
    imageUrl = generateUrl(`api/frames/bookies/${FrameNames.PLACE_BET}/image`, {[RequestProps.PROMPT]: event.prompt, [RequestProps.BALANCE]: user.balance, [RequestProps.TIME]: event.startDate}, true);
  }
  ...
  const frame : Frame = {
    ...
    image: imageUrl,
    postUrl: generateUrl(`api/frames/bookies/${FrameNames.BETSLIP}`, {[RequestProps.EVENT_NAME]: eventName}, false),
  };
