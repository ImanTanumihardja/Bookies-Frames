...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const imageUrl = generateUrl(`api/frames/bookies/${FrameNames.BET_CONFIRMATION}/image`, {[RequestProps.STAKE]: stake, [RequestProps.PICK]: pick, [RequestProps.BUTTON_INDEX]: button, [RequestProps.FID]: fid, [RequestProps.EVENT_NAME]: eventName, [RequestProps.OPTIONS]: event.options, [RequestProps.TIME]: now, [RequestProps.RESULT]: event.result}, true);
    ...
    getFrameHtml({
      ...
      [
        ...
        }, 
        {
          ...
          action:'post', 
          target: generateUrl(`/api/frames/bookies/${FrameNames.PLACE_BET}`, {[RequestProps.EVENT_NAME]: eventName}, false)}
        ],
      postUrl: `${process.env['HOST']}/api/frames/bookies/${FrameNames.BET_CONFIRMATION}`,
    }),
  );
}
