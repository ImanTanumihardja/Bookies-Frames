...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (event.startDate < now || result !== -1) {
    ...
    imageUrl = generateUrl(`api/bookies/${FrameNames.EVENT}/${FrameNames.BET_CONFIRMATION}/image`, {[RequestProps.STAKE]: 0, 
  ...
  else 
  {
    buttons = event.options.map((option, index) => {
      ...
      return {
        ...
        action: 'tx',
        target: generateUrl(`api/bookies/${FrameNames.EVENT}/${FrameNames.TRANSACTIONS}/${FrameNames.APPROVE}`, {}, false),
      } as FrameButton
    })
  ...
  const frame : Frame = {
    ...
    image: imageUrl,
    postUrl: generateUrl(`api/bookies/${FrameNames.EVENT}/${FrameNames.BETSLIP}`, {[RequestProps.EVENT_NAME]: eventName}, false),
  };
