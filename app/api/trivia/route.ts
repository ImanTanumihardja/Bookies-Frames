...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (user?.strikes >= 3) { // You ran out of strikes
    // Game over
    postUrl = `${process.env['HOST']}/api/${FrameNames.TRIVIA}?count=${count}&index=${correctIndex}`
  }
    ...
    // End quiz
    count = -1;
    postUrl = `${process.env['HOST']}/api/${FrameNames.TRIVIA}?count=-1&array=${EASY_QUESTION_INDEXES}`
  }
  else { // Continue game
    ...
    postUrl = `${process.env['HOST']}/api/${FrameNames.TRIVIA}?count=${count}&index=${correctIndex}&array=${encodeURIComponent(questionIndexes.toString())}`
  }

  const imageUrl = generateUrl(`api/${FrameNames.TRIVIA}/image`, {[RequestProps.IS_FOLLOWING]: isFollowing, [RequestProps.PROMPT]: question, [RequestProps.WINS]: count, [RequestProps.LOSSES]: user.strikes}, true);
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const frame : Frame = {
    ...
    image: imageUrl,
    postUrl: `${process.env['HOST']}/api/${FrameNames.TRIVIA}?count=-1&array=${questionIndexes}`,
  };
